...
- **No cuentes la historia del documento, cuenta el estado actual.** La web es para la propietaria, no un registro de cómo ha evolucionado el análisis: nada de "corrección importante", "versiones anteriores tenían mal", "dato ya resuelto", "cifra retirada en esta revisión" ni explicaciones de qué decía antes este plan. Publica el dato correcto y ya está. Esto **no afecta** a las marcas de incertidumbre de los puntos anteriores: seguir avisando de que algo es fuente única o está sin confirmar es informar sobre el dato, no confesar un error pasado.
