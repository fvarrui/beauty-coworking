# Plan económico-financiero

**Estado: provisional y parcial.** Hay cifras reales de ingresos por alquiler, pero ningún coste fijo del local está confirmado todavía — esto no es un descuido, es que la propietaria no los ha facilitado aún (ver huecos al final). No se ha estimado ni redondeado ningún coste para poder cerrar una tabla completa: donde falta el dato, la fila queda en blanco y marcada como pendiente.

## 1. Ingresos

### 1.1 Por alquiler de espacio (el núcleo del modelo)

| Espacio | Estado | Ingreso mensual |
|---|---|---|
| Salón central (profesional de pedicura) | **Ya real, en marcha** | 300 € |
| Cubículo 1 | Objetivo — ocupado hoy por la propietaria, se libera con la transición | ~400 € |
| Cubículo 2 | Objetivo — ya libre | ~400 € |
| **Total a pleno rendimiento** | | **~1.100 €/mes** |

Cifras de [`diversificacion.md`](../negocio/diversificacion.md) y [`precios-servicios.md`](../negocio/precios-servicios.md). Los ~400 €/mes de cada cubículo son un precio objetivo, no cerrado — ver pregunta abierta en [`pendientes.md`](../negocio/pendientes.md).

### 1.2 Bruto facturado vs. neto cobrado — matiz importante

Estas cifras son el importe del alquiler, pero **no es lo que la propietaria recibe en la cuenta**:

- Sobre el alquiler se aplica **IGIC del 7%**, no IVA del 21%: Canarias está fuera del territorio de aplicación del IVA. La propietaria lo repercute en factura y lo declara con el modelo 420 ante la Agencia Tributaria Canaria; no se lo queda. ⚠️ Además, con este volumen de facturación probablemente pueda acogerse al **REPEP** y no repercutir IGIC en absoluto — ver [`tramites-administrativos.md`](../negocio/tramites-administrativos.md#4-igic-e-irpf-del-alquiler-de-las-cabinas).
- Cada inquilina **retiene un 19% de IRPF** sobre el alquiler (sin IGIC) y lo ingresa directamente a cuenta de la propietaria (modelo 115).
- **Caja y rentabilidad no son lo mismo**: esa retención **no es un gasto**, es un pago anticipado del IRPF de la propietaria, que se descuenta de su declaración anual. Afecta a la caja mes a mes, no a la rentabilidad del negocio. Lo mismo el IGIC, que solo pasa por su cuenta camino de la Agencia Tributaria Canaria.

Con un alquiler de 400 € de base, las tres cifras que conviene no confundir son:

| Concepto | Importe | Qué es |
|---|---|---|
| Base de la factura | 400 € | El ingreso real del negocio, y la cifra que debe usarse en este documento |
| Total facturado | 428 € | Base + 7% de IGIC (0 € de IGIC si se acoge al REPEP) |
| Cobro en cuenta | 352 € | Total facturado − 19% de retención sobre la base (324 € si no hay IGIC) |

Sigue pendiente aclarar con la propietaria si los ~400 €/mes que maneja son la base de factura o lo que espera ver en la cuenta: entre una lectura y otra hay un 12% de diferencia de caja.

Detalle completo del cálculo en [`tramites-administrativos.md`](../negocio/tramites-administrativos.md#4-iva-e-irpf-del-alquiler-de-las-cabinas). **Pendiente aclarar con la propietaria si los ~400 €/mes objetivo que se manejan en todo el plan son el importe bruto de la factura o lo que espera cobrar neto** — cambia la cifra real de ingreso en más de un 15%.

### 1.3 Ingresos por servicios propios (mientras la propietaria sigue ejerciendo)

Sin datos: la ficha de precios de estética/depilación y de pedicura está vacía — ver [`precios-servicios.md`](../negocio/precios-servicios.md). Sin precio por servicio ni volumen de clientas, **no se puede estimar la facturación actual de la propietaria ni de la profesional de pedicura**, y por tanto tampoco:
- Cuánto ingreso deja de tener la propietaria durante el periodo de transición (cuando deje el Cubículo 1 y aún no esté cubierto por un nuevo inquilino).
- El valor real de la cartera de clientas que va a traspasar — importante porque es un activo que entrega como parte del trato de entrada (ver [`contrato-alquiler-cabina.md`](../negocio/contrato-alquiler-cabina.md)).

## 2. Costes

### 2.1 Costes fijos mensuales del local

Ningún coste fijo está confirmado todavía. Categorías habituales para un local de este tipo, pendientes de que la propietaria las facilite:

| Concepto | Importe mensual | Estado |
|---|---|---|
| Suministros de zonas comunes (agua, luz, internet) | — | ⚠️ Pendiente de confirmar con la propietaria |
| Seguro multirriesgo del local + RC como arrendadora | — | ⚠️ Pendiente — ver [`tramites-administrativos.md`](../negocio/tramites-administrativos.md#7-seguro-del-local); sin cotizar todavía con un corredor |
| Gestoría / asesoría fiscal | — | ⚠️ Pendiente de confirmar si ya tiene una contratada |
| IBI (impuesto de bienes inmuebles) | — | ⚠️ Pendiente |
| Comunidad de propietarios (si aplica) | — | ⚠️ Pendiente de confirmar si el local tiene cuota de comunidad |
| Mantenimiento/limpieza de zonas comunes | — | ⚠️ Pendiente — responsabilidad de la propietaria según [`contrato-alquiler-cabina.md`](../negocio/contrato-alquiler-cabina.md), coste aún sin estimar |

No hay hipoteca ni alquiler del local que pagar (es propiedad, ver [`local.md`](../negocio/local.md)), lo que reduce el número de partidas fijas frente a un local alquilado — pero las de arriba siguen sin cuantificarse.

### 2.2 Costes puntuales de puesta en marcha

| Concepto | Importe | Estado |
|---|---|---|
| Equipamiento/reposición de los cubículos | — | ⚠️ Depende del inventario pendiente en [`pendientes.md`](../negocio/pendientes.md) — no se sabe qué hay que reponer hasta no inventariar cada espacio |
| Redacción del contrato de alquiler de cabina | — | ⚠️ Coste de gestor/abogado, sin presupuestar |
| Consulta a la Oficina de Licencias de Arona | Probablemente gratuita (trámite administrativo) | Ver [`tramites-administrativos.md`](../negocio/tramites-administrativos.md#1-licencia-de-actividad-municipal-ayuntamiento-de-arona) |

**Ayuda pública disponible para el equipamiento** (no para los costes fijos recurrentes): la línea del Cabildo cubre el 60% de la inversión en equipamiento hasta 10.000 €/local, pero el plazo 2026 ya cerró — es anual, hay que vigilar la convocatoria 2027. Detalle completo en [`ayudas-subvenciones.md`](../negocio/ayudas-subvenciones.md#2-nivel-insular-cabildo-de-tenerife).

## 3. Punto de equilibrio

**No se puede calcular todavía de forma real** — solo con ingresos conocidos y ningún coste fijo confirmado, cualquier cifra de punto de equilibrio sería ilustrativa, no un cálculo de verdad, y daría una falsa sensación de precisión. Lo que sí se puede decir con lo que hay:

- El **salón central ya cubre, por sí solo, 300 €/mes de ingreso real** — si los costes fijos totales del local resultan ser inferiores a esa cifra, el modelo ya sería rentable solo con ese puesto ocupado, antes incluso de alquilar los 2 cubículos.
- En cuanto la propietaria facilite aunque sea una cifra aproximada de costes fijos mensuales, este apartado se puede completar con un punto de equilibrio real (cuántos puestos alquilados hacen falta para cubrir gastos).

## 4. Escenarios

Solo dos, ambos con datos reales de ingresos (nada de escenarios optimista/pesimista inventados sin base):

| Escenario | Ingreso bruto mensual | Qué representa |
|---|---|---|
| **Solo salón central** | 300 €/mes | Situación actual, ya real y en marcha — el "suelo" del modelo, sin necesidad de captar ninguna inquilina nueva |
| **A pleno rendimiento** | ~1.100 €/mes | Los 3 puestos alquilados (salón central + Cubículo 1 + Cubículo 2) — el objetivo del plan, sin plazo definido todavía |

Ninguno de los dos descuenta costes fijos (sección 2, sin confirmar) — son cifras brutas de ingreso por alquiler, no beneficio neto.

### 4.1. La comparación que falta: el plan frente a seguir como hasta ahora

Los dos escenarios de arriba comparan 300 € con 1.100 €, como si el punto de partida fuera un local medio vacío. No lo es: hoy la propietaria, además de cobrar los 300 €, **ejerce y factura** en el Cubículo 1. La comparación correcta es otra:

| Escenario | Ingreso mensual |
|---|---|
| **Hoy** | 300 € + facturación propia de la propietaria |
| **Plan completo** | ~1.100 € |
| **Diferencia** | 800 € − facturación propia |

Es decir: **el cambio de modelo solo mejora sus ingresos si su actividad actual le deja menos de 800 € netos al mes.** Si le deja más, el plan completo la empobrece.

**Dato bloqueante**: sin la facturación propia de la propietaria (sección 1.3) no se puede afirmar que el plan sea económicamente favorable, solo que aumenta los ingresos por alquiler. Es la cifra que más condiciona la decisión de todo el plan, por encima incluso de los costes fijos.

**Un factor que juega a favor del cambio y que no estaba contabilizado**: si al dejar de ejercer su ingreso pasa a calificarse como *rendimiento de capital inmobiliario*, [alquilar un local no obliga por sí solo a estar de alta en el RETA](https://declarando.es/blog/ingresos-arrendamiento), con lo que ahorraría la cuota de autónoma — desde [205 €/mes en 2026](https://www.infoautonomos.com/seguridad-social/cuota-de-autonomos-cuanto-se-paga/), del orden de 2.500 €/año sobre unos ingresos brutos de 13.200 €. ⚠️ Depende de qué incluya el alquiler: la [DGT considera actividad económica el alquiler de espacios con servicios adicionales](https://ga-p.com/publicaciones/aspectos-fiscales-relativos-al-alquiler-de-espacios-en-centros-de-negocios-o-coworking/) (recepción, agenda, limpieza, material), y en ese caso seguiría obligada a cotizar. Es un argumento económico más a favor del alquiler mensual pelado ya decidido (ver [`propuesta-oferta-coworking.md`](../negocio/propuesta-oferta-coworking.md#6-modelo-de-precio-alquiler-mensual-en-los-dos-cubículos)). Confirmar con asesor fiscal.

## 5. Qué haría falta para que este documento deje de ser provisional

Por orden de impacto en el cálculo:

1. **Costes fijos mensuales del local** (suministros, seguro, gestoría, IBI, comunidad) — es el dato que más cambiaría este documento; sin él no hay punto de equilibrio real.
2. **Facturación propia actual de la propietaria** — sin ella no se puede saber si el cambio de modelo mejora o empeora sus ingresos (ver sección 4.1); es, junto con los costes fijos, el dato que decide.
3. **Si los ~400 €/mes objetivo de cada cubículo son base de factura o cobro en cuenta** — un 12% de diferencia de caja (ver sección 1.2).
4. **Precios de los servicios propios de la propietaria y de la profesional de pedicura** — necesario para estimar la facturación actual y el ingreso que se pierde durante la transición.
5. **Inventario de equipamiento de cada espacio** — para poder presupuestar qué falta reponer antes de alquilar los cubículos.
6. **Plazos deseados para cubrir los 2 cubículos** — sin esto no se puede proyectar un calendario de ingresos a 12 meses, solo el estado "a pleno rendimiento" sin fecha.

Una vez la propietaria aporte aunque sea el primero de estos puntos, conviene regenerar la presentación con `/presentar` para que la sección "Los números" quede sincronizada con este documento.
